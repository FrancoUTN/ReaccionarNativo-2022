  ...
  function onCantidadModificadaHandler(id, cantidad) {
    ...
    if (productoExistente) {
      if (cantidad === 0) {
        setProductosSeleccionados((productosPrevios) => {
          return productosPrevios.filter(
            (indice) => indice.producto.id !== productoExistente.producto.id
          );
        });
      } else {
        setProductosSeleccionados((productosPrevios) => {
      ...
      if (cantidad === 0) {
        ...
        const productoNuevo = productos.find((prod) => prod.id == id);
  ...
  return productos.length > 0 ? (
    <>
      ...
      <View style={styles.viewImporte}>
        <Text style={styles.textImporte}>Total: $ {importe}</Text>
        <View style={styles.viewDemora}>
          <Ionicons name="time-outline" color="white" size={26} />
          <Text style={styles.textDemora}>{demora}'</Text>
        </View>
        <Pressable
          style={({ pressed }) => [
            styles.pressablePedir,
            pressed && { opacity: 0.7 },
          ]}
